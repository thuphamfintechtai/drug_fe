      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-cyan-200 shadow-sm p-5">
            ...
            <p className="text-slate-500 text-sm mt-1">Xem thông tin tài khoản và nhà thuốc (chỉ đọc)</p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-5">
            ...
            {profile.pharmacy && (
              <div className="bg-white rounded-2xl border border-cyan-100 shadow-sm overflow-hidden">
                <div className="px-6 py-4 bg-gradient-to-r from-[#00b4d8] to-[#48cae4]">
                  <h2 className="text-xl font-bold text-white">Thông tin nhà thuốc</h2>
                </div>
                <div className="p-6 space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-cyan-50 rounded-xl p-4 border border-cyan-200">
                      <div className="text-xs text-cyan-700 mb-1">Tên nhà thuốc</div>
                      <div className="font-bold text-cyan-900 text-lg">{profile.pharmacy.name || 'N/A'}</div>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-4">
                      <div className="text-xs text-slate-500 mb-1">Mã số thuế</div>
                      <div className="font-mono font-semibold text-slate-800">{profile.pharmacy.taxCode || 'N/A'}</div>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-4">
                      <div className="text-xs text-slate-500 mb-1">Giấy phép kinh doanh</div>
                      <div className="font-mono font-semibold text-slate-800">{profile.pharmacy.licenseNo || 'N/A'}</div>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-4">
                      <div className="text-xs text-slate-500 mb-1">Trạng thái</div>
                      <div className="font-semibold text-slate-800">
                  ...
                  <div className="bg-slate-50 rounded-xl p-4">
                    ...
                    <div className="font-medium text-slate-800">{profile.pharmacy.address || 'N/A'}</div>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="bg-slate-50 rounded-xl p-4">
                      <div className="text-xs text-slate-500 mb-1">Số điện thoại</div>
                      <div className="font-medium text-slate-800">{profile.pharmacy.contactPhone || 'N/A'}</div>
                    </div>
                    <div className="bg-slate-50 rounded-xl p-4">
                      <div className="text-xs text-slate-500 mb-1">Email liên hệ</div>
                      <div className="font-medium text-slate-800">{profile.pharmacy.contactEmail || 'N/A'}</div>
                    </div>
                  </div>

                  {profile.pharmacy.walletAddress && (
                    <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-200">
                      <div className="text-xs text-emerald-700 mb-1">Wallet Address</div>
            ...
            <div className="bg-yellow-50 rounded-2xl border border-yellow-200 p-5">
              ...
              <div className="text-sm text-yellow-700">
                ...
                Nếu cần thay đổi thông tin nhà thuốc, vui lòng liên hệ với quản trị viên hệ thống.
              </div>
            </div>
          </div>
        </div>
