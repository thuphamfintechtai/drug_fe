    ...
    <DashboardLayout navigationItems={navigationItems}>
      ...
      <section className="relative overflow-hidden rounded-2xl border border-[#90e0ef33] shadow-[0_10px_30px_rgba(0,0,0,0.06)] bg-gradient-to-tr from-[#00b4d8] via-[#48cae4] to-[#90e0ef]">
        ...
        <div className="relative px-6 py-8 md:px-10 md:py-12 text-white">
          <h1 className="text-2xl md:text-3xl font-semibold tracking-tight drop-shadow-sm">Tạo Proof of Pharmacy</h1>
          <p className="mt-2 text-white/90">Nhập thông tin biên nhận nhập hàng để xác thực minh bạch.</p>
        </div>
      </section>
      ...
      <div className="mt-5 rounded-2xl bg-white/85 backdrop-blur-xl border border-[#90e0ef55] shadow-[0_10px_30px_rgba(0,0,0,0.06)] p-5">
        <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input className="border border-[#90e0ef55] bg-white/60 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#48cae4] focus:border-[#48cae4]" placeholder="Proof of Distribution ID" value={form.proofOfDistributionId} onChange={(e) => setForm({ ...form, proofOfDistributionId: e.target.value })} />
          <input className="border border-[#90e0ef55] bg-white/60 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#48cae4] focus:border-[#48cae4]" placeholder="NFT Info ID" value={form.nftInfoId} onChange={(e) => setForm({ ...form, nftInfoId: e.target.value })} />
          <input className="border border-[#90e0ef55] bg-white/60 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#48cae4] focus:border-[#48cae4]" placeholder="Drug ID" value={form.drugId} onChange={(e) => setForm({ ...form, drugId: e.target.value })} />
          <input className="border border-[#90e0ef55] bg-white/60 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#48cae4] focus:border-[#48cae4]" placeholder="Số lượng nhận" value={form.receivedQuantity} onChange={(e) => setForm({ ...form, receivedQuantity: e.target.value })} />
          <input className="border border-[#90e0ef55] bg-white/60 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#48cae4] focus:border-[#48cae4]" placeholder="Địa chỉ nhận" value={form.receiptAddress} onChange={(e) => setForm({ ...form, receiptAddress: e.target.value })} />
          ...
          <div className="md:col-span-2">
            <button disabled={loading} className="px-4 py-2.5 rounded-xl text-white bg-gradient-to-r from-[#00b4d8] via-[#48cae4] to-[#90e0ef] shadow-[0_10px_24px_rgba(0,180,216,0.30)] hover:shadow-[0_14px_36px_rgba(0,180,216,0.40)] disabled:opacity-50 transition">Tạo</button>
          </div>
        </form>
      </div>
