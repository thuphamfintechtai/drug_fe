...
export default function ProofList() {
  ...
  const loadProofs = async (page = 1) => {
    setLoading(true);
    try {
      const response = await getMyProofs(page, 10);
      if (response.success) {
      ...
      alert('Không thể tải danh sách proofs');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-800">Proof of Production</h1>
                <p className="text-sm text-gray-500">Danh sách chứng nhận sản xuất</p>
              </div>
            </div>
            <button
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
          {loading ? (
            ...
            <div className="p-12 flex flex-col items-center text-center">
              ...
              <h3 className="text-xl font-bold text-gray-800 mb-2">Chưa có Proof nào</h3>
              <p className="text-gray-600 mb-4">Hãy tạo Proof of Production đầu tiên</p>
              <button
                onClick={() => navigate('/manufacturer/proofs/create')}
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-gradient-to-r from-cyan-600 to-teal-600">
                  <tr>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-white uppercase">Mã lô</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-white uppercase">Tên thuốc</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-white uppercase">Số lượng</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-white uppercase">Ngày sản xuất</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-white uppercase">Ngày hết hạn</th>
                    <th className="px-6 py-4 text-left text-sm font-semibold text-white uppercase">Ngày tạo</th>
                    <th className="px-6 py-4 text-center text-sm font-semibold text-white uppercase">Thao tác</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {proofs.map((proof, index) => (
                    <tr key={proof._id || index} className="hover:bg-gradient-to-r hover:from-cyan-50 hover:to-teal-50 transition-all duration-200 group">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex flex-col">
                          ...
                          {proof.drug?.genericName && (
                            <span className="text-sm text-gray-500 mt-1">{proof.drug.genericName}</span>
                          )}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="font-bold text-gray-800">{proof.quantity?.toLocaleString() || 0}</span>
                        <span className="text-sm text-gray-500 ml-2">viên</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-700">
                          {proof.mfgDate ? new Date(proof.mfgDate).toLocaleDateString('vi-VN') : 'N/A'}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-gray-700">
                          {proof.expDate ? new Date(proof.expDate).toLocaleDateString('vi-VN') : 'N/A'}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex flex-col">
                          <span className="text-sm text-gray-700">
                            {proof.createdAt ? new Date(proof.createdAt).toLocaleDateString('vi-VN') : 'N/A'}
                          </span>
                          {proof.createdAt && (
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-100">
              <div className="flex justify-between items-center">
                ...
                <div className="text-gray-700">
                  Trang <span className="font-bold text-cyan-600">{currentPage}</span> / {pagination.pages}
                  {' • '}
