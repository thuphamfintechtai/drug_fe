...
export default function ManufacturerIPFSStatus() {
  ...
  const getStatusBadge = (status) => {
    ...
    if (s.includes("success") || s === "done") {
      return { text: "Thành công", cls: "bg-emerald-50 text-emerald-700 border-emerald-200" };
    }
    if (s.includes("pending") || s.includes("processing")) {
      return { text: "Đang xử lý", cls: "bg-amber-50 text-amber-700 border-amber-200" };
    }
    if (s.includes("fail") || s.includes("error")) {
      return { text: "Lỗi", cls: "bg-rose-50 text-rose-700 border-rose-200" };
    }
    return { text: status || "Không rõ", cls: "bg-slate-50 text-slate-700 border-slate-200" };
  };
    ...
    <DashboardLayout navigationItems={navigationItems}>
      {loading ? (
        <div className="flex flex-col items-center justify-center min-h-[70vh]">
          ...
          <div className="text-lg text-slate-600 mt-6">Đang tải lịch sử IPFS...</div>
        </div>
      ) : (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-card-primary shadow-sm overflow-hidden">
            {items.length === 0 ? (
              <div className="p-16 text-center text-slate-500">Không có dữ liệu</div>
            ) : (
                ...
                <table className="w-full border-collapse">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-sm font-semibold text-gray-700">IPFS</th>
                    ...
                    {pageItems.map((item, idx) => {
                      ...
                      return (
                        <tr key={idx} className="hover:bg-gray-50 transition-colors">
                          <td className="px-6 py-4 text-gray-700">{formatDate(item.createdAt || item.timestamp || item.time)}</td>
                          ...
                          <td className="px-6 py-4 text-gray-700">{item.quantity || item.amount || item.total || "—"}</td>
                          <td className="px-6 py-4">
                            ...
                            {(String(item.status || "").toLowerCase().includes("pending") ||
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  if (item.ipfsUrl) q.set("ipfsUrl", item.ipfsUrl);
                                  if (item.quantity) q.set("quantity", String(item.quantity));
                                  navigate(`/manufacturer/production?${q.toString()}`);
                                }}
          ...
          {items.length > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
              <div className="text-sm text-slate-600">
                Hiển thị {start + 1}-{Math.min(start + pageSize, total)} trên {total}
              </div>
