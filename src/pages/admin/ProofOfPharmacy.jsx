    ...
    <DashboardLayout navigationItems={navigationItems}>
      ...
      >
        ...
        <div className="relative px-6 py-8 md:px-10 md:py-12 text-white">
          <h2 className="text-2xl md:text-3xl font-semibold tracking-tight drop-shadow-sm">Proof of Pharmacy</h2>
          <p className="text-white/90 mt-1">Minh chứng nhà thuốc – xác thực minh bạch.</p>
        </div>
      </motion.section>
            ...
            <tbody>
              {items.map(p => (
                <tr key={p._id} className="border-t border-[#90e0ef40] hover:bg-[#f5fcff] transition">
                  ...
                  <td className="p-3 text-[#003544]/80">{p.pharmacyName || p.pharmacy?.name}</td>
                  <td className="p-3 text-[#003544]/80">{p.status}</td>
                  <td className="p-3 text-right">
                    <a href={`/admin/proof-of-pharmacy/${p._id}`} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-[#90e0ef55] text-[#003544] hover:bg-[#90e0ef22] transition">Chi tiết
