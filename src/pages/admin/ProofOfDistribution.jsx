    ...
    <DashboardLayout navigationItems={navigationItems}>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          />
          <div>
            <h2 className="text-lg font-semibold text-slate-800">Proof of Distribution</h2>
            <p className="text-sm text-slate-600">Minh chứng phân phối – xác thực chuỗi cung ứng minh bạch</p>
          </div>
        </div>
      </motion.div>
            ...
            <tbody>
              {items.map(p => (
                <tr key={p._id} className="border-t hover:bg-slate-50/60 transition">
                  ...
                  <td className="p-3 text-slate-700">{p.distributorName || p.distributor?.name}</td>
                  <td className="p-3 text-slate-700">{p.status}</td>
                  <td className="p-3 text-right">
                    <a href={`/admin/proof-of-distribution/${p._id}`} className="px-3 py-2 rounded-lg border border-slate-200 hover:border-cyan-300 hover:bg-cyan-50 text-slate-700">Chi tiết</a>
                  </td>
                </tr>
              ))}
              {items.length === 0 && (
