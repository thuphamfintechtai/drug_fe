      ...
      <div className="bg-white rounded shadow overflow-x-auto">
        {loading ? <div className="p-6">Đang tải...</div> : error ? <div className="p-6 text-red-600">{error}</div> : (
          <table className="min-w-full">
            <thead><tr className="bg-gray-50 text-left"><th className="p-3">Drug</th><th className="p-3">Distributor</th><th className="p-3">Status</th></tr></thead>
            <tbody>
              {items.map(p => (
                <tr key={p._id} className="border-t"><td className="p-3">{p.drugName || p.drug?.name}</td><td className="p-3">{p.distributorName || p.distributor?.name}</td><td className="p-3">{p.status}</td></tr>
              ))}
