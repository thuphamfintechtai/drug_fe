...
export default function TransferToPharmacy() {
  ...
  const extractTokenIds = (distributionObj) => {
    ...
    if (distributionObj.manufacturerInvoice?.tokenIds && Array.isArray(distributionObj.manufacturerInvoice.tokenIds)) {
      return distributionObj.manufacturerInvoice.tokenIds.map(id => String(id));
    }
    ...
    if (distributionObj.nftInfos && Array.isArray(distributionObj.nftInfos)) {
      const tokenIds = distributionObj.nftInfos.map(nft => {
    ...
    try {
          ...
          try {
            ...
            if (invoiceDetailRes?.data?.success && invoiceDetailRes.data.data) {
              ...
              if (invoiceDetail.tokenIds && Array.isArray(invoiceDetail.tokenIds) && invoiceDetail.tokenIds.length > 0) {
                ...
                console.warn('⚠️ API getInvoiceDetail không trả về tokenIds:', invoiceDetail);
              }
            }
          } catch (invoiceError) {
      ...
      if (tokenIds.length === 0) {
        ...
        alert(
          ...
          `Invoice ID: ${dist?.manufacturerInvoice?._id || dist?.manufacturerInvoice || 'N/A'}\n\n` +
          `Vui lòng kiểm tra:\n` +
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-semibold text-slate-800">Tạo invoice</div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <div className="font-semibold text-slate-800">Lưu transaction hash</div>
                ...
                <div className="p-8 space-y-4 relative">
                  ...
                  {selectedPharmacy && (
                    <div className="bg-cyan-50 rounded-xl p-4 border border-cyan-200">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm">
                        ...
                        <div><span className="text-slate-600">Email:</span> <span className="font-medium">{selectedPharmacy.contactEmail || 'N/A'}</span></div>
                        <div><span className="text-slate-600">SĐT:</span> <span className="font-medium">{selectedPharmacy.contactPhone || 'N/A'}</span></div>
                        <div className="md:col-span-2"><span className="text-slate-600">Wallet:</span> <span className="font-mono text-xs break-all">{selectedPharmacy.walletAddress || selectedPharmacy.user?.walletAddress || 'Chưa có'}</span></div>
                      </div>
                  ...
                  <div className="bg-yellow-50 rounded-xl p-4 border border-yellow-200">
                    <div className="text-sm text-yellow-800">
                      ...
                      NFT sẽ được chuyển on-chain ngay lập tức.
                    </div>
                  </div>
                </div>
