...
export default function PublicDrugInfo() {
  ...
  const [searchType, setSearchType] = useState(searchParams.get('type') || 'name'); // 'name' or 'atc'
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        ...
        setError('Một số thông tin thuốc yêu cầu đăng nhập để xem. Vui lòng đăng nhập để xem đầy đủ thông tin.');
      } else {
        setError(error.response?.data?.message || error.message || 'Không thể tra cứu thông tin thuốc.');
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
        ...
        >
          ...
          <div className="flex gap-3">
            <input
              ...
              placeholder={searchType === 'atc' ? 'Nhập mã ATC (ví dụ: A01AA01)...' : 'Nhập tên thuốc...'}
                  ...
                  <div className="flex items-start gap-2 mb-2">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-blue-700 mb-3">
                        Đăng nhập để xem đầy đủ thông tin thuốc bao gồm: thành phần chi tiết, hướng dẫn sử dụng, tương tác thuốc, và thông tin nhà sản xuất.
                      </p>
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => {
                            toast.info('Vui lòng liên hệ quản trị viên để được cấp tài khoản đăng nhập');
                          }}
          ...
          {isAuthenticated && (
            <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg text-green-800 text-sm flex items-center gap-2">
              <span>✅</span>
              <span>Bạn đã đăng nhập với tư cách <strong>{user?.role === 'system_admin' ? 'Quản trị viên' : user?.role === 'pharma_company' ? 'Nhà sản xuất' : user?.role === 'distributor' ? 'Nhà phân phối' : user?.role === 'pharmacy' ? 'Nhà thuốc' : 'Người dùng'}</strong>. Bạn có thể xem đầy đủ thông tin.</span>
            </div>
          )}
        </motion.div>
        ...
        {loading ? (
          <div className="bg-white rounded-2xl border border-slate-200 p-10 text-center">
            <div className="text-xl text-slate-600">Đang tìm kiếm thông tin thuốc...</div>
          </div>
        ) : !searched ? (
          <motion.div 
          ...
          >
            ...
            <p className="text-slate-600 max-w-md mx-auto">
              Nhập tên thuốc hoặc mã ATC vào ô tìm kiếm phía trên để xem thông tin chi tiết
            </p>
          </motion.div>
        ) : drugs.length === 0 ? (
          <motion.div 
          ...
          >
            <div className="text-6xl mb-4">❌</div>
            <h3 className="text-2xl font-bold text-red-600 mb-2">Không tìm thấy</h3>
            <p className="text-slate-600">Không tìm thấy thông tin thuốc phù hợp. Vui lòng thử lại với từ khóa khác.</p>
          </motion.div>
        ) : (
          <motion.div className="space-y-4" variants={fadeUp} initial="hidden" animate="show">
            {drugs.map((drug, idx) => (
              <div key={idx} className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md transition">
                  ...
                  <div>
                    ...
                    {drug.genericName && drug.tradeName && (
                      <p className="text-slate-600 mb-2">
                        <span className="font-semibold">Tên hoạt chất:</span> {drug.genericName}
                      </p>
                    )}
                    {drug.atcCode && (
                      <p className="text-slate-600 mb-2">
                        <span className="font-semibold">Mã ATC:</span> <span className="font-mono">{drug.atcCode}</span>
                      </p>
                    )}
                    {!isAuthenticated && (
                  ...
                  <div className="space-y-2">
                    {drug.dosageForm && (
                      <div className="bg-slate-50 rounded-lg p-3">
                        <div className="text-sm text-slate-500">Dạng bào chế</div>
                        <div className="font-semibold text-slate-800">{drug.dosageForm}</div>
                      </div>
                    )}
                    {drug.strength && (
                      <div className="bg-slate-50 rounded-lg p-3">
                        <div className="text-sm text-slate-500">Hàm lượng</div>
                        <div className="font-semibold text-slate-800">{drug.strength}</div>
                      </div>
                    )}
                    {drug.packaging && (
                      <div className="bg-slate-50 rounded-lg p-3">
                        <div className="text-sm text-slate-500">Quy cách đóng gói</div>
                        <div className="font-semibold text-slate-800">{drug.packaging}</div>
                      </div>
                    )}
                    {drug.manufacturer && (
                      <div className="bg-slate-50 rounded-lg p-3">
                        <div className="text-sm text-slate-500">Nhà sản xuất</div>
                        <div className="font-semibold text-slate-800">
                          {typeof drug.manufacturer === 'object' 
                            ? (drug.manufacturer.name || drug.manufacturer.fullName || 'N/A')
                            : drug.manufacturer}
                        </div>
                      </div>
                    )}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {drug.route && (
                        <div className="bg-blue-50 rounded-lg p-3 border border-blue-200">
                          <div className="text-sm text-blue-700">Đường dùng</div>
                          <div className="font-semibold text-blue-900">{drug.route}</div>
                        </div>
                      )}
                      {drug.storage && (
                        <div className="bg-blue-50 rounded-lg p-3 border border-blue-200">
                          <div className="text-sm text-blue-700">Bảo quản</div>
                          <div className="font-semibold text-blue-900">{drug.storage}</div>
                        </div>
                      )}
                      {drug.activeIngredients && Array.isArray(drug.activeIngredients) && drug.activeIngredients.length > 0 && (
                        <div className="bg-blue-50 rounded-lg p-3 border border-blue-200 md:col-span-2">
                          <div className="text-sm text-blue-700 mb-1">Thành phần hoạt chất</div>
                      ...
                      {drug.warnings && (
                        <div className="bg-amber-50 rounded-lg p-3 border border-amber-200 md:col-span-2">
                          <div className="text-sm text-amber-700 mb-1">⚠️ Cảnh báo</div>
                          <div className="text-sm text-amber-900">{drug.warnings}</div>
                        </div>
                      )}
                      {drug.indications && (
                        <div className="bg-green-50 rounded-lg p-3 border border-green-200 md:col-span-2">
                          <div className="text-sm text-green-700 mb-1">📋 Chỉ định</div>
                          <div className="text-sm text-green-900">{drug.indications}</div>
                        </div>
                      )}
                      {drug.contraindications && (
                        <div className="bg-red-50 rounded-lg p-3 border border-red-200 md:col-span-2">
                          <div className="text-sm text-red-700 mb-1">🚫 Chống chỉ định</div>
                          <div className="text-sm text-red-900">{drug.contraindications}</div>
                        </div>
                      )}
                    </div>
                  </div>
