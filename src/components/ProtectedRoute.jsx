...
const ProtectedRoute = ({ children, allowedRoles = [] }) => {
  ...
  useEffect(() => {
    const checkWalletAddress = async () => {
      ...
      if (!isAuthenticated || !user?.walletAddress || loading || checkingWallet) {
      ...
      if (!isInstalled) {
        ...
        toast.error('Vui lòng cài đặt MetaMask extension để truy cập trang này.');
        return;
      }
      ...
      if (!compareWalletAddresses(user.walletAddress, account)) {
        ...
        toast.error(`Địa chỉ ví MetaMask không khớp. Yêu cầu: ${requiredAddress}`);
        return;
      }
  ...
  }, [isAuthenticated, user?.walletAddress, account, isConnected, isInstalled, loading]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#4BADD1]/5 via-white to-slate-50/50 px-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl border border-red-200 p-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Địa chỉ ví không khớp</h1>
          <p className="text-slate-600 mb-4">
            Tài khoản của bạn yêu cầu địa chỉ ví MetaMask: <span className="font-mono font-semibold text-[#4BADD1]">{formatWalletAddress(user.walletAddress)}</span>
          </p>
          {account && (
            <p className="text-sm text-slate-500 mb-6">
              Địa chỉ ví hiện tại: <span className="font-mono">{formatWalletAddress(account)}</span>
            </p>
          )}
          <div className="flex flex-col gap-3">
            ...
            <a
              href="/"
              className="text-sm text-[#4BADD1] hover:underline"
            >
              Về trang chủ
            </a>
          </div>
