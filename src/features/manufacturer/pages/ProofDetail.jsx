            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-800">
                  Chi tiết Proof of Production
                </h1>
                <p className="text-sm text-gray-500">
                  Mã lô: {proof.batchNumber || "N/A"}
                </p>
              </div>
            </div>
            <button
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6">
            ...
            <div className="space-y-4">
              <InfoRow
                label="Tên thuốc"
                value={proof.drug?.tradeName || proof.drugName || "N/A"}
              />
              <InfoRow
                label="Tên hoạt chất"
                value={proof.drug?.genericName || proof.genericName || "N/A"}
              />
              <InfoRow label="Mã ATC" value={proof.drug?.atcCode || "N/A"} />
              <InfoRow label="Mã lô" value={proof.batchNumber || "N/A"} badge />
              <InfoRow
                label="Serial Number"
                value={proof.serialNumber || nftInfo?.serialNumber || "N/A"}
                mono
              />
              <InfoRow
                label="Số lượng"
                value={`${proof.quantity?.toLocaleString() || 0} viên`}
              />
              <InfoRow
                label="Ngày sản xuất"
                value={
                  proof.mfgDate
                    ? new Date(proof.mfgDate).toLocaleDateString("vi-VN")
                    : "N/A"
                }
              />
              <InfoRow
                ...
                value={
                  proof.expDate
                    ? new Date(proof.expDate).toLocaleDateString("vi-VN")
                    : "N/A"
                }
              />
              <InfoRow
                label="Kiểm định viên"
                value={proof.qaInspector || "Không có"}
              />
              {proof.qaReportUri && (
                <div className="pt-2">
                  <a
            ...
            {nftInfo ? (
              <div className="space-y-4">
                ...
                <InfoRow
                  ...
                  value={
                    nftInfo.contractAddress || proof.contractAddress || "N/A"
                  }
                  mono
                  small
                />
                <InfoRow
                  label="Batch Number"
                  value={nftInfo.batchNumber || "N/A"}
                  badge
                />
                <InfoRow
                  label="Status"
                  value={nftInfo.status || "minted"}
                  status
                />
                <InfoRow
                  label="Owner"
                  value={
                    nftInfo.owner?.fullName || nftInfo.owner?.username || "N/A"
                  }
                />
...
const InfoRow = ({ label, value, badge, mono, small, status }) => (
  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-2 border-b border-gray-100">
    <span className="text-sm font-semibold text-gray-600 mb-1 sm:mb-0">
      {label}:
    </span>
    {badge ? (
