...
export const useDrugDetail = () => {
  ...
  const navigationItems = useMemo(
    () => [
      { path: "/distributor", label: "Tổng quan", icon: null, active: false },
      {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        // Lấy danh sách drugs và tìm drug theo id
        const response = await api.get(`/distributor/drugs`);
        const drugs = response.data?.data?.drugs || [];
        const found = drugs.find((item) => {
