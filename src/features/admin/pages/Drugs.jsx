          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 mb-6">
            <div className="relative flex-1 w-full">
              ...
              <input
                ...
                placeholder="Tìm theo mã ATC"
                className="w-full h-12 pl-11 pr-28 sm:pr-40 rounded-full border border-gray-200 bg-white text-gray-700 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#48cae4] transition text-sm sm:text-base"
              />
              {searchInput && (
                <button
                  type="button"
                  onClick={handleClearSearch}
                  className="absolute right-20 sm:right-24 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition p-1"
                  title="Xóa tìm kiếm"
                >
                  <svg
                    className="w-4 h-4"
              ...
              <button
                ...
                className="absolute right-1 top-1 bottom-1 px-3 sm:px-6 rounded-full bg-secondary hover:bg-primary !text-white font-medium transition text-xs sm:text-sm whitespace-nowrap"
              >
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
              <button
                ...
                className="px-3 sm:px-4 py-2.5 rounded-full border border-gray-300 text-slate-700 hover:bg-gray-50 transition text-sm whitespace-nowrap"
              >
                Reset
              </button>
              <select
                ...
                className="h-12 rounded-full border border-gray-200 bg-white text-gray-700 px-3 sm:px-4 pr-6 sm:pr-8 focus:outline-none focus:ring-2 focus:ring-[#48cae4] transition text-sm sm:text-base"
              >
          ...
          <div className="bg-white rounded-2xl border border-card-primary shadow-sm overflow-hidden mb-6">
            {error ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse border-card-primary min-w-[640px]">
                  <thead className="bg-gray-50 border-b border-gray-100">
                    <tr>
                      <th className="px-3 sm:px-6 py-3 text-left text-xs sm:text-sm font-semibold text-gray-700">
                        Tên thương mại
                      </th>
                      <th className="px-3 sm:px-6 py-3 text-left text-xs sm:text-sm font-semibold text-gray-700 hidden md:table-cell">
                        Tên hoạt chất
                      </th>
                      <th className="px-3 sm:px-6 py-3 text-left text-xs sm:text-sm font-semibold text-gray-700">
                        Mã ATC
                      </th>
                      <th className="px-3 sm:px-6 py-3 text-left text-xs sm:text-sm font-semibold text-gray-700 hidden lg:table-cell">
                        Nhà sản xuất
                      </th>
                      <th className="px-3 sm:px-6 py-3 text-left text-xs sm:text-sm font-semibold text-gray-700">
                        Trạng thái
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {items.map((d) => (
                      ...
                      >
                        <td className="px-3 sm:px-6 py-4 font-medium text-gray-800 text-sm">
                          <div className="flex flex-col">
                            <span>{d.tradeName}</span>
                            <span className="text-xs text-gray-500 md:hidden mt-1">
                              {d.genericName || "-"}
                            </span>
                          </div>
                        </td>
                        <td className="px-3 sm:px-6 py-4 text-gray-600 text-sm hidden md:table-cell">
                          {d.genericName || "-"}
                        </td>
                        <td className="px-3 sm:px-6 py-4">
                          <span className="inline-flex items-center px-2 sm:px-2.5 py-1 rounded-md text-xs font-mono font-semibold bg-cyan-50 text-cyan-700 border border-cyan-100">
                            {d.atcCode}
                          </span>
                        </td>
                        <td className="px-3 sm:px-6 py-4 text-gray-600 text-sm hidden lg:table-cell">
              ...
              <button
                ...
                className={`px-3 sm:px-4 py-2 rounded-xl text-sm transition ${
                  page <= 1
                    ...
                    : "bg-white border border-primary hover:bg-[#f5fcff] text-primary"
                }`}
              >
                Trước
              </button>
              <span className="text-sm text-slate-700 px-2">Trang {page}</span>
              <button
                onClick={() => updateFilter({ page: page + 1 })}
                disabled={
                  page >= (pagination?.totalPages || 1) ||
                  items.length < (pagination?.limit || 10)
                }
                className={`px-3 sm:px-4 py-2 rounded-xl text-sm text-white bg-gradient-to-r from-secondary to-primary shadow-[0_10px_24px_rgba(0,180,216,0.30)] hover:shadow-[0_14px_36px_rgba(0,180,216,0.40)] transition disabled:opacity-50 disabled:cursor-not-allowed`}
              >
                Sau
              </button>
            </div>
